...
class AtualizadorDados:
    ...
    def _carregar_dados_csv(self):
        ...
        if dados_csv:
            ...
            # Definindo dtype como str para preservar zeros à esquerda
            dados = pd.read_csv(dados_io, names=self.nomes_colunas, skiprows=1, dtype=str)
            # Convertendo a coluna 'Concurso' de string para int após a leitura
        ...
        while True:
            ...
            try:
                ...
                if opcao == 1:
                    ...
                    self._auto_completar()
                elif opcao == 4:
                    logging.info("Encerrando o programa.")
                    break
                else:
                    logging.warning("Opção inválida. Escolha uma opção de 1 a 4.")
            except ValueError:
                logging.error("Por favor, digite um número válido.")

if __name__ == "__main__":
    caminho_pasta = '/home/henrique/PycharmProjects/extração'
